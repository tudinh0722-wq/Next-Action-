...
class PlannerScreen extends StatefulWidget {
  const PlannerScreen({super.key, required this.events, required this.database, required this.scheduler, required this.tts, this.themeMode = ThemeMode.system, this.seedColor = const Color(0xFF1A73E8), this.onThemeChanged, this.onSeedColorChanged});
...
class _PlannerScreenState extends State<PlannerScreen> {
  ...
  void initState() {
    ...
    _recurrenceService = RecurrenceService(database: widget.database, scheduler: widget.scheduler);
    _bulkImportService = BulkImportService(database: widget.database, scheduler: widget.scheduler);
  ...
  void _shiftMonth(int delta) {
    ...
    setState(() { _month = target; _selected = DateTime(target.year, target.month, _selected.day.clamp(1, maxDay)); });
  }
  ...
  void _handleVerticalSwipe(DragEndDetails details) {
    ...
    else if (velocity > 0 && !_expanded) setState(() => _expanded = true);
  }

  Future<void> _reloadEvents() async {
    final events = await widget.database.getAll();
    if (!mounted) return;
    setState(() { _events = events; _events.sort((a, b) => a.start.compareTo(b.start)); });
  }

  Future<void> _editEvent(NextAEvent? event) async {
    final result = await showEventEditor(context, event: event, selectedDay: _selected, importService: event == null ? _bulkImportService : null);
    if (!mounted) return;
    if (result == null) { await _reloadEvents(); return; }
    ...
    if (event != null) {
      ...
      final editResult = await _recurrenceService.edit(event, edited, scope, _events);
      if (!mounted) return;
      setState(() {
    ...
    for (final e in result.events) { await widget.scheduler.scheduleEvent(e); }
    if (!mounted) return;
    setState(() { _events.addAll(result.events); _events.sort((a, b) => a.start.compareTo(b.start)); });
  }

  Future<void> _addEventForDay(DateTime day) async {
    final result = await showEventEditor(context, selectedDay: DateTime(day.year, day.month, day.day), importService: _bulkImportService);
  ...
  String _selectedHeader() { const weekdays = ['T.2', 'T.3', 'T.4', 'T.5', 'T.6', 'T.7', 'CN']; return '${_selected.day}   ${weekdays[_selected.weekday - 1]}'; }
  ...
  Widget build(BuildContext context) {
    return Scaffold(body: SafeArea(child: Column(children: [
      ...
      Expanded(child: GestureDetector(behavior: HitTestBehavior.translucent, onVerticalDragEnd: _handleVerticalSwipe, child: Column(children: [
        GestureDetector(behavior: HitTestBehavior.opaque, onHorizontalDragEnd: (details) { final velocity = details.primaryVelocity ?? 0; if (velocity.abs() > 200) { _expanded ? _shiftMonth(velocity < 0 ? 1 : -1) : _shiftWeek(velocity < 0 ? 1 : -1); } }, child: PlannerCalendar(month: _month, selected: _selected, expanded: _expanded, eventsFor: _eventsFor, onSelect: _selectDay, onLongPress: _addEventForDay, animationDuration: _pageAnimationDuration)),
        Expanded(child: GestureDetector(behavior: HitTestBehavior.opaque, onHorizontalDragEnd: (details) { final velocity = details.primaryVelocity ?? 0; if (velocity.abs() > 200) _shiftDay(velocity < 0 ? 1 : -1); }, child: Stack(children: [
  ...
  Future<void> _showThemeMenu(BuildContext context) async {
    await showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheetContext) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(16, 4, 16, 20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _ScopeDialog extends StatefulWidget {
  ...
  @override State<_ScopeDialog> createState() => _ScopeDialogState();
}
class _ScopeDialogState extends State<_ScopeDialog> {
  RecurrenceScope _scope = RecurrenceScope.single;
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return AlertDialog(title: Text(widget.title), content: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    ]), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Hủy')), widget.isDestructive ? FilledButton(style: FilledButton.styleFrom(backgroundColor: scheme.error, foregroundColor: scheme.onError), onPressed: () => Navigator.pop(context, _scope), child: Text(widget.confirmLabel)) : FilledButton(onPressed: () => Navigator.pop(context, _scope), child: Text(widget.confirmLabel))]);
  }
}
...
class _SearchDialog extends StatefulWidget {
  ...
  final EventDatabase database;
  @override State<_SearchDialog> createState() => _SearchDialogState();
}
class _SearchDialogState extends State<_SearchDialog> {
  ...
  void _search() { _debounce?.cancel(); _debounce = Timer(const Duration(milliseconds: 180), () async { final results = await widget.database.search(_controller.text); if (mounted) setState(() => _results = results); }); }
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Dialog(insetPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 24), child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 430, maxHeight: 560), child: SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 18, 20, 12), child: Column(children: [
      ...
      const SizedBox(height: 8), TextField(controller: _controller, autofocus: true, onChanged: (_) => _search(), decoration: const InputDecoration(prefixIcon: Icon(Icons.search_rounded), hintText: 'Tên, địa điểm hoặc ghi chú')), const SizedBox(height: 12),
...
class _SeedColorButton extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => GestureDetector(onTap: () { onSelected?.call(color); Navigator.pop(context); }, child: CircleAvatar(radius: 17, backgroundColor: color));
}
...
class PlannerTopBar extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return SizedBox(height: 64, child: Row(children: [IconButton(onPressed: onMenu, icon: const Icon(Icons.menu_rounded)), const Spacer(), Text(monthLabel, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w700)), const Spacer(), IconButton(onPressed: onSearch, icon: const Icon(Icons.search_rounded)), Padding(padding: const EdgeInsets.only(right: 8), child: Material(color: Colors.transparent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), child: InkWell(onTap: onToday, borderRadius: BorderRadius.circular(10), child: Container(width: 38, height: 38, alignment: Alignment.center, decoration: BoxDecoration(border: Border.all(color: scheme.outline, width: 1.5), borderRadius: BorderRadius.circular(10)), child: Text('$today', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800)))))]));
  }
}
...
class PlannerFab extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Material(elevation: 3, shadowColor: scheme.shadow.withValues(alpha: .18), color: scheme.surfaceContainerHighest, shape: const StadiumBorder(), child: InkWell(onTap: onTap, customBorder: const StadiumBorder(), child: SizedBox(height: 60, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.add_rounded, size: 18), const SizedBox(width: 6), Flexible(child: Text(label, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)))]))));
  }
}
