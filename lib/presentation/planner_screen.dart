...
class _PlannerScreenState extends State<PlannerScreen> {
  ...
  void _startTicker() {
    ...
    _countdownTimer = Timer(Duration(seconds: 60 - DateTime.now().second), () {
      if (!mounted) return;
      setState(() {});
  ...
  void _shiftWeek(int delta) => _shiftDay(delta * 7);
  ...
  Future<void> _editEvent(NextAEvent? event) async {
    ...
    if (event != null) {
      final scope = result.scope ?? RecurrenceScope.single;
      final editResult = await _recurrenceService.edit(event, result.events.first, scope, _events);
      if (!mounted) return;
      setState(() {
            ...
            child: GestureDetector(
              ...
              child: Column(children: [
                GestureDetector(
                  ...
                  child: PlannerCalendar(month: _month, selected: _selected, expanded: _expanded, eventsFor: _eventsFor, onSelect: _selectDay, onLongPress: _addEventForDay, animationDuration: const Duration(milliseconds: 420)),
                ),
                Expanded(
                  child: GestureDetector(
                    behavior: HitTestBehavior.opaque,
                    onHorizontalDragEnd: (details) { final velocity = details.primaryVelocity ?? 0; if (velocity.abs() > 200) _shiftDay(velocity < 0 ? 1 : -1); },
  ...
  Future<void> _showThemeMenu(BuildContext context) async {
    await showModalBottomSheet<void>(
      context: context,
      showDragHandle: true,
      builder: (c) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(16, 4, 16, 20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 10, children: [for (final color in const [Color(0xFF1A73E8), Color(0xFF6750A4), Color(0xFF006A6A), Color(0xFF8E4A2F), Color(0xFF7A4E00)]) _SeedColorButton(color: color, onSelected: widget.onSeedColorChanged)]),
      ]))),
    );
  }
}
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: Column(children: [
              ...
              Expanded(child: _results.isEmpty ? Center(child: Text('Không tìm thấy sự kiện', style: TextStyle(color: scheme.onSurfaceVariant))) : ListView.separated(padding: EdgeInsets.zero, itemCount: _results.length, separatorBuilder: (_, __) => const Divider(height: 1), itemBuilder: (_, i) { final e = _results[i]; return ListTile(title: Text(e.title, maxLines: 1, overflow: TextOverflow.ellipsis), subtitle: Text('${e.start.day}/${e.start.month}/${e.start.year} · ${e.location ?? 'Không có địa điểm'}'), onTap: () => Navigator.pop(context, e)); })),
            ]),
          ),
        ),
      ),
